...
voms_mapfile_script = "%s/gen-voms-mapfile" % os.path.dirname(__file__)
if not os.path.exists(voms_mapfile_script):
    print >>sys.stderr, "voms mapfile script not found," \
                        " won't report custom VOMS mappings."
    sys.exit()

p = subprocess.Popen([voms_mapfile_script, gumsconfig], stdout=subprocess.PIPE)
current_voms_maps, _ = p.communicate()
...
def txt_to_lines(txt):
    ...
    return filter(nonempty, lines)

released_voms_maps = txt_to_lines(released_voms_maps)
current_voms_maps  = txt_to_lines(current_voms_maps)

custom_voms_maps = set(current_voms_maps) - set(released_voms_maps)

if custom_voms_maps:
    print "# custom VOMS mappings"
    for m in current_voms_maps:
        if m in custom_voms_maps:
            print m
    print
